...
func (r *CloudClusterReconciler) provisionKeycloakAccount(ctx context.Context, cluster *cloudv1beta1.CloudCluster) (*ctrl.Result, error) {
	log := log.FromContext(ctx)

	// Check if the keyclock credential secret exists.
	...
	// Provision keyclock credential if it doesn't exist
	if cluster.Status.KeycloakCredential == nil || cluster.Status.KeycloakClientID == nil {
		...
		token, err := gocloakClient.LoginClient(
			ctx,
		...
		redirectURIS := []string{
			cluster.Name + "." + cluster.Spec.ProjectID + ".skc.cloud.seda.club",
		}

		clients, err := gocloakClient.GetClients(ctx, token.AccessToken, realm, gocloak.GetClientsParams{
			ClientID: &clientID,
		})
		if err != nil {
		...
		if len(clients) != 0 {
			...
			id, err := gocloakClient.CreateClient(ctx, token.AccessToken, realm, keycloakClient)
			if err != nil {
		...
		credentialRepresentatoin, err := gocloakClient.RegenerateClientSecret(ctx, token.AccessToken, realm, *keycloakClient.ID)
		if err != nil {
...
func (r *CloudClusterReconciler) ProvisionVCluster(ctx context.Context, cluster *cloudv1beta1.CloudCluster) (*ctrl.Result, error) {
	log := log.FromContext(ctx)
	...
	if cluster.Status.KeycloakCredential == nil {
		...
		return &ctrl.Result{}, errors.NewGone("gone with the winds")
	}
	...
	if cluster.Status.ClusterName != &apicluster.Name {
...
func (r *CloudClusterReconciler) ProvisionTLSTunnel(ctx context.Context, cluster *cloudv1beta1.CloudCluster) (*ctrl.Result, error) {
	log := log.FromContext(ctx)
	if cluster.Status.AddressName != nil {
		...
		addr := cloudv1beta1.NetworkAddress{
			...
			Spec: cloudv1beta1.NetworkAddressSpec{
				Type:            "domain",
				Address:         []string{cluster.Name + "." + cluster.Spec.ProjectID + ".skc.cloud.seda.club"},
		...
		if err := r.Client.Status().Update(ctx, cluster); err != nil {
			log.Error(err, "An error occured while saving CloudCluster CR")
			return &ctrl.Result{}, err
		} // if it fails, it will be updated on enxt reconcile run.
	...
	if cluster.Status.BindingName != nil {
		...
		addr := cloudv1beta1.NetworkAddressBinding{
			...
			Spec: cloudv1beta1.NetworkAddressBindingSpec{
				...
				Address:             cluster.Name + "." + cluster.Spec.ProjectID + ".skc.cloud.seda.club",
...
func (r *CloudClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if result, err := r.provisionKeycloakAccount(ctx, &cluster); result != nil {
		return *result, err
	}

	// Provision vcluster ofc
	if result, err := r.ProvisionVCluster(ctx, &cluster); result != nil {
		return *result, err
	}

	// Provision tls tunnel ofc
	if result, err := r.ProvisionTLSTunnel(ctx, &cluster); result != nil {
		return *result, err
	}
